...
class HttpRequestVerifierTest extends TestCase
{
    ...
    public function testShouldReturnExpectedTokenIfTokenSetToRequestAttribute(): void
    {
        ...
        $request = Request::create('https://target.com/foo');
        $request->query->set('payum_token', $expectedToken->getHash());
    ...
    {
        $this->expectException(HttpException::class);
        $this->expectExceptionMessage('The current url https://target.com/foo not match target url https://target.com/bar set in the token.');

        $expectedToken = new Token();
        $expectedToken->setHash('theHash');
        $expectedToken->setTargetUrl('https://target.com/bar');
